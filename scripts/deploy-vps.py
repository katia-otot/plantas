...
swap. El servicio sigue activo durante el build: solo se para un momento antes
del swap. Si el build falla, la app no queda en 502.
...
def try_start_plantas(client: paramiko.SSHClient) -> None:
    """Best-effort: never leave producción caída tras un deploy fallido."""
    try:
        run(client, "systemctl start plantas || systemctl restart plantas || true")
        run(client, "systemctl is-active plantas || true")
    except Exception as exc:  # noqa: BLE001
        print(f"WARN: no se pudo reactivar plantas: {exc}", file=sys.stderr)
...
def main() -> None:
    ...
    # .env del VPS suele apuntar a file:/opt/plantas/data/app.db (absoluto).
    # Durante build/migrate hay que forzar la copia en .new para no tocar la DB viva.
    new_db_url = f"file:{new_dir}/data/app.db"
    ...
    try:
        # 1. Backup de app.db con el servicio AÚN activo (cp es suficiente acá)
        run(
            client,
        ...
        # 3. Extraer en .new, copiar data/.env/secrets (producción sigue sirviendo)
        run(
            client,
        ...
        # 4. Verificar DB de la copia antes de build
        run(
            client,
        ...
        # 5. Build en .new — producción sigue UP. Prisma solo toca la copia.
        run(
            client,
            f"cd {new_dir} && "
            "chmod 755 scripts/notify-today.sh 2>/dev/null || true && "
            "npm install && "
            f"DATABASE_URL='{new_db_url}' NEXT_PUBLIC_BASE_PATH=/plantas "
            "npx prisma db push && "
            f"DATABASE_URL='{new_db_url}' npx tsx scripts/migrate-multi-garden.ts && "
            "NEXT_PUBLIC_BASE_PATH=/plantas npm run build && "
            "test -f .next/BUILD_ID",
            timeout=1800,
        )

        # 6. Downtime corto: parar → refrescar data → swap → arrancar
        ...
        run(client, "systemctl start plantas")
        service_stopped = False
        time.sleep(4)
        ...
        # 7. Quitar cron viejo de notify-today (scheduler interno en la app)
        run(
            client,
        ...
        if service_stopped:
            print(
                "Deploy falló con servicio parado — reactivando producción…",
                file=sys.stderr,
            )
            try_start_plantas(client)
        raise
    finally:
        client.close()
